...
- **AC20d** `✅` **— Máy quầy quà không quét badge; lịch sử SV không bịa badge** (DIỄN TẬP 11/09). `record_scan` ghi một dòng attendance cho **mọi** điểm và chỉ bỏ qua bước cộng `badge_count` khi `counts_toward_badges = false` — nên dòng "Quầy đổi quà" là thật, còn "+1" là do `/api/toi` đọc `badge_weight` (mặc định 1) mà không hỏi cờ đó. Cùng lỗi đó sẽ hiện "Cổng check-in +1" cho **mọi** SV qua cổng ngày 12/09 trong khi con số badge to phía trên đứng yên. Sửa ba chỗ: `lib/experience.js` chỉ cho `+N` khi điểm thật sự tính badge, bỏ `gift_counter`/`info_desk` khỏi lịch sử, giữ cổng với nhãn "Check-in" (7 test); `/quet` trên máy đứng ở Quầy đổi quà **không ghi lượt nào** mà hiện hộp thoại "MỞ QUẦY QUÀ" kèm dải vàng trên camera — vì thẻ xanh "ĐÃ GHI NHẬN" ở quầy quà là cách dễ nhất để PG tưởng đã trao quà; và dòng phụ trên thẻ xanh thôi hứa "badge thứ N+1" ở cổng, cộng đúng trọng số 3–4 (`localScanMeta`, 7 test). Không đổi DB, không cần migration.
- **AC20e** `✅` **— Quầy quà và quầy vé quét được người tiếp theo** (video DIỄN TẬP 11/09). Triệu chứng: quét SV thứ nhất, bấm NGƯỜI TIẾP THEO, khung camera đen; vài giây sau thẻ của **chính SV đó** tự hiện lại, lặp mãi. `/qua` và `/suat` gọi `startScanner()` nhưng chưa bao giờ giữ handle nó trả về, nên `scannerRef.current?.stop()` gọi vào `null`: camera không tắt, và vòng giải mã cũ cứ chạy trên thẻ `<video>` đã bị gỡ khỏi màn hình — hết mỗi cooldown 2,5 giây nó đọc lại mã của người vừa rồi. `/quet` không dính vì gán handle từ ngày đầu. Có từ trước 10/09; lộ ra vì đây là lần đầu hai quầy online được test thật. Chữa bằng `lib/desk-camera.js` dùng chung cho cả hai quầy (14 test): đúng một handle và `stop()` tắt thật; mỗi `stop()` mở một thế hệ mới, callback của thế hệ cũ bị bỏ; `stop()` lúc camera còn đang khởi động thì camera đó tắt ngay khi sáng; đang tra một mã thì dừng giải mã, tra xong mà vẫn ở màn quét (mã sai, mất mạng) thì chạy tiếp nhưng giữ cooldown (`scanner.resume({ keepLast })`) để hộp thoại lỗi không bật liên hồi. Camera nay sáng **khi và chỉ khi** khung quét đang hiện, và tự bật lại khi bấm NGƯỜI TIẾP THEO.
- **AC20f** `✅` **— PHÁT trước, đưa sau** (ảnh DIỄN TẬP 11/09 18:18). SV Lê Minh Kha đủ 9 badge; sổ chỉ có dòng Hộp bút lúc 18:09, **không** có dòng Túi quà, nên lần quét lại máy — đúng theo sổ — mời "TRAO THÊM Túi quà". Nguyên nhân sổ thiếu dòng túi đang chẩn đoán (`ATL2026-SQL-Chan-doan-quay-qua.sql`: hàm 0014 đã cài chưa, chế độ thang, kho lúc đó, audit). Nhưng điều biến nó thành **mất đồ thật** là lỗi thiết kế của AC20c: màn hình dặn "đưa đủ rồi mới bấm xác nhận" — tức đồ rời bàn **trước** khi sổ xác nhận, ngược với chính nguyên tắc ghi ở đầu `qua/page.js`. Nay: thẻ quyết định ghi **SẼ TRAO** (dự kiến) → nút **PHÁT QUÀ** → ô giữa màn hình **ĐƯA CHO SV** đọc từ `granted` (dòng server vừa ghi) → PG đưa đúng những món đó. Món dự định mà sổ không ghi → ô chuyển hổ phách, dòng "KHÔNG đưa: …". `grantOutcome()` trong `lib/gift-plan.js` (5 test). Nhờ vậy đồ trên tay SV luôn khớp sổ, **kể cả khi** nguyên nhân phía server là gì.
- **AC20g** `✅` **— Thang quà khoá cứng ở Cộng dồn** (nguyên nhân của AC20f, chẩn đoán 11/09). Hàm 0014 đã cài đúng (một chữ ký, `security definer`, `search_path=public`). Thủ phạm là cấu hình: **TP.HCM đang ở `highest_only`** — audit cho thấy nút "Đổi" ở dòng Thang quà bị bấm **11 lần** lúc 11:49–11:52 ngày 10/09 và dừng ở "Bậc cao nhất", trước cả khi AIM chốt quy định; Hà Nội bị bấm tương tự lúc 13:16 nhưng dừng ở Cộng dồn. Ở `highest_only` hàm ghi đúng một món và tắt cấp kèm, nên hai SV (GXT4FT 18:09, 5SSETX 18:12, cùng máy QUA-01) có hộp bút mà sổ không có túi. Chế độ đó mâu thuẫn trực tiếp với quy định AIM, nên nay **không chọn được nữa**, ở cả bốn lớp: tab Cấu hình bỏ nút hai chiều, chỉ còn nút một chiều "Đặt lại Cộng dồn" hiện khi sự kiện đang sai; `PATCH /api/admin/config` trả 409 cho `highest_only`; `POST /api/pg/gift` từ chối phát (`wrong_ladder_mode`) khi sự kiện không ở Cộng dồn — kể cả với máy PG còn chạy bản cũ; màn `/qua` có trạng thái `misconfigured` — khung đỏ, không hứa món nào, không có nút phát (2 test). Hàm SQL giữ nguyên nhánh `highest_only` cho sự kiện khác sau này, nhưng ATL2026 không đi vào nó được nữa.
  **Dọn dữ liệu test TP.HCM — 11/09 tối (anh Hoàng chạy, sau khi xem trước).** Đo trước: Hà Nội sạch hoàn toàn; TP.HCM có test từ 17:30 tới 18:21 trên máy thật — 23 lượt của 4 SV, 2 lượt đổi quà, 2 ghế hội trường. Một khối SQL duy nhất (tự từ chối nếu chạy từ 12/09; đã chạy thử trên PGlite đủ 14 migration, gồm cả ca "ngày mai quét lại vẫn được badge"): thang quà về Cộng dồn · xoá 2 lượt đổi quà và trả kho · trả 2 ghế · **huỷ** (không xoá) 23 lượt `attendance` — `ledger_events` giữ nguyên làm bằng chứng, và vì `attendance_one_per_checkpoint` chỉ tính dòng chưa huỷ nên lượt thật ngày 12/09 vẫn được cộng · dựng lại bộ đếm 4 hồ sơ · xoá 23 dòng `checkpoint_minute_counts` để giờ test không lọt vào Excel nhà tài trợ. Mọi kiểm tra sau dọn = 0. Vết trong `audit_log`: action `cleanup_test_data`, actor `sql:don-du-lieu-test`. **Còn lại ngoài tầm SQL:** hàng đợi trên từng máy PG chặn trùng theo (SV, điểm) với mọi lượt đã lưu, kể cả lượt đã xác nhận — nên nếu 4 SV đó là SV thật, máy đã test sẽ tự báo "đã có badge" khi quét lại họ ở đúng điểm đó.
...
- **AC28a** `✅` **— Khoá sự kiện** (AIM 11/09: "khoá/ẩn DIỄN TẬP để ngày mai không bị rối"). Cột `events.is_archived` (0015). Sự kiện đã khoá biến khỏi `/api/refdata` — tức khỏi form đăng ký online, form **tại cổng** và ô chọn sự kiện của admin; ô chọn tự về điểm đầu tiên nếu điểm đã lưu là sự kiện đã khoá, để không hiện chữ "Hà Nội" trên số liệu diễn tập. `/api/register` từ chối — trước đó danh sách hợp lệ viết cứng `[1, 2, 3]`, nên tại cổng gửi DIỄN TẬP lên là server nhận, và một SV thật cầm mã QR diễn tập thì quét ở đâu cũng đỏ. Trang lịch lọc theo cờ khoá thay cho `is_registration_open`, nên AIM đóng đăng ký online cũng không còn làm Hà Nội và TP.HCM biến khỏi trang lịch. App đọc cờ qua `to_jsonb(e)->>'is_archived'` (`lib/event-visibility.js`) để chạy được cả trước khi 0015 được áp (6 test, trên schema thật trước và sau khi có cột). Máy PG của sự kiện khoá không cần code riêng: thu hồi hết máy là đủ — `claim_pg_device` từ chối mã đã thu hồi, còn `resolve_pg_device` biến token cũ thành 403 → màn "BTC đã thu hồi mã". **Lối ra cho điện thoại đã bị thu hồi** (phát hiện khi viết lại sổ tay PG 11/09): trang nhận máy chỉ hiện "Máy đã sẵn sàng" khi điện thoại còn phiên cũ, còn màn thu hồi không có nút nào ngoài xem hàng đợi — nên điện thoại đã tập dượt không nhập được mã máy thật của ngày sự kiện, trừ khi xoá dữ liệu trang web trong cài đặt trình duyệt. Màn thu hồi nay có nút **NHẬN MÃ MÁY MỚI** → hỏi xác nhận kèm số lượt chưa gửi → `forgetRevokedDevice()` xoá phiên, điểm quét, danh sách SV và hàng đợi → về ô nhập mã. Chỉ có ở màn thu hồi, tức sau khi server đã trả 403: token cũ đã chết nên hàng đợi vốn không gửi lên được, và giữ lại thì chúng sẽ đi lên bằng token của máy mới với điểm quét của sự kiện cũ. Máy còn sống vẫn giữ lời dặn "đừng tự nhập mã mới".
  **Sự cố tối 11/09 — đăng ký trả 500.** Bản khoá sự kiện (95c6f26) ghép `${EVENT_NOT_ARCHIVED}` vào câu SQL của `/api/register` mà quên import. `next build` không bắt được — biến chưa khai báo trong JavaScript chỉ nổ lúc chạy — nên từ lần deploy đó, **mọi lượt đăng ký, online lẫn tại cổng**, trả 500 và form hiện "Mạng đang chập chờn"; SV và AIM báo lúc 23:22. Lỗi nổ sau bước kiểm dữ liệu và trước `register_student`, nên không có bản ghi dở dang hay email gửi nhầm — người gặp lỗi chỉ cần bấm Đăng ký lại. Sửa ở 6c03fce, xác nhận trên production bằng một yêu cầu thử vào sự kiện đã khoá (400 thay vì 500, không ghi gì). Chặn tái phạm: `apps/web/test/undeclared-sql-constants.test.js` quét mã nguồn hai app — mọi `${HANG_SO}` ghép vào template literal phải được import hoặc khai báo trong chính file đó; bài kiểm tự thử lại đúng đoạn mã lỗi để chắc nó bắt được.
...
- **AC31** `✅` — Thang quà cộng dồn hay chỉ bậc cao nhất. *(11/09: khoá cứng ở Cộng dồn theo quy định AIM — xem AC20g. Tab Cấu hình chỉ còn nút một chiều "Đặt lại Cộng dồn", có audit.)*
